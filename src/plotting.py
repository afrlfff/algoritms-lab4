...
file = open(os.path.join(OUTPUT_DIR, "results.txt"), 'r')
lines = file.readlines()

column_names = lines[0].split(); lines.pop(0)
FILENAMES = [line.split()[0].split('.')[0] for line in lines]
ENTROPY_RATIOS = [float(line.split()[1]) for line in lines]
DECODING_RATIOS = [float(line.split()[2]) for line in lines]
COMPRESSION_RATIOS = [float(line.split()[3]) for line in lines]
START_SIZES = [float(line.split()[4]) for line in lines]
ENCODED_SIZES = [float(line.split()[5]) for line in lines]
